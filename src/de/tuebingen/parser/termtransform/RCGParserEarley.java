...
public class RCGParserEarley extends RCGParser {
    ...
    public RCGParserEarley(Grammar g) {
        super(g);
    ...
    public boolean recognize(List<Word> input) {
        ...
        return true;// || earleyparse(goalitem, Tokenizer.tok2string(input));
    }

    //private
    boolean earleyparse(Item goalitem, List<String> input) {
        ...
        while (!found && !stop) {
            ...
            while (agendait.hasNext()) {
                ...
                if (item.getClause().getRhs().size() == 0) {
                    ...
                    //item.dotpos < item.getClause().getRhs().size()) {
...
//		g.copylanguage();
        ...
        g.calcAllRangeConstraintVectors();
        System.err.println(g.print());
        Tokenizer tok = new BuiltinTokenizer();
        tok.setSentence("a a");
        List<Word> input = tok.tokenize();
        RCGParserEarley p = new RCGParserEarley(g);
