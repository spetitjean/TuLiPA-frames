...
public class SlimTAGParser {
	...
	if (pos == 1) {
		...
		if (nChildren == 1) {
		    ...
		    Iterator<Integer> it = closed_map.get(child1).get(i).keySet().iterator();
		    
		    while (it.hasNext()) {

			int split_point = it.next().intValue();
						
			double hyp = closed_map.get(child1).get(i).get(split_point)[0] 
			    + closed_map.get(child2).get(split_point).get(j)[0];
		...
		for (TagTree subst_tag_tree : initial_trees.get(category)) {
		    ...
		    //this is currently superfluous, we already ensured matching categories
		    ...
		    if (closed_map.get(subst_root).get(i) != null && closed_map.get(subst_root).get(i).get(j) != null)
			hyp = closed_map.get(subst_root).get(i).get(j)[0]
		...
		for (TagTree subst_tag_tree : auxiliary_trees.get(category)) {
		    ...
		    Iterator<Integer> it_i = closed_map.get(cur_node).keySet().iterator();
			...
			Iterator<Integer> it_j = closed_map.get(cur_node).get(i).keySet().iterator();
			...
			while (it_j.hasNext()) {
			    ...
			    if (gap_map.get(subst_root).get(i).get(i_inner).get(j_inner) == null
				|| gap_map.get(subst_root).get(i).get(i_inner).get(j_inner).get(j) == null)
				continue;
			    ...
			    // of the tree. This way the system with the passes is less complicated.
			    // In the end we have to add the weight of the initial tree
			    double hyp = gap_map.get(subst_root).get(i).get(i_inner).get(j_inner).get(j)[0]
				+ closed_map.get(cur_node).get(i_inner).get(j_inner)[1]
				+ 1.0; //this is the tree weight
	...
	//System.err.println("trace_all_with_gap("+i1+","+i2+","+j1+","+j2+","+pos + ") for " +cur_node.toString());
	...
	if (pos == 1) {
		...
		if (nChildren == 1) {
			...
			if (gap_map.get(child1).get(i1) != null
			    ...
			    && gap_map.get(child1).get(i1).get(i2).get(j1) != null) {
			
			    Iterator<Integer> it = gap_map.get(child1).get(i1).get(i2).get(j1).keySet().iterator();
				...
				if (closed_map.get(child2).get(split_point) != null
				    && closed_map.get(child2).get(split_point).get(j2) != null) 
				    hyp = gap_map.get(child1).get(i1).get(i2).get(j1).get(split_point)[0]
					+ closed_map.get(child2).get(split_point).get(j2)[0];
			...
			Iterator<Integer> it = closed_map.get(child1).get(i1).keySet().iterator();
			...
			while (it.hasNext()) {
				...
				&& gap_map.get(child2).get(split_point).get(i2) != null
				&& gap_map.get(child2).get(split_point).get(i2).get(j1) != null
				&& gap_map.get(child2).get(split_point).get(i2).get(j1).get(j2) != null) 
				hyp = closed_map.get(child1).get(i1).get(split_point)[0]
				    + gap_map.get(child2).get(split_point).get(i2).get(j1).get(j2)[0];
	    ...
	    if (cur_node.getAdjStatus() != TagNode.MADJ) {
		//adjunction is not obligatory
		double hyp_na = gap_map.get(cur_node).get(i1).get(i2).get(j1).get(j2)[1];
		if (hyp_na < 1e300) {
		...
		for (TagTree subst_tag_tree : auxiliary_trees.get(category)) {
			...
			Iterator<Integer> it_i = gap_map.get(subst_root).get(i1).keySet().iterator();
			...
			while(it_i.hasNext()) {
				...
				|| gap_map.get(cur_node).get(i_inter).get(i2) == null
				|| gap_map.get(cur_node).get(i_inter).get(i2).get(j1) == null)
				continue;

			    Iterator<Integer> it_j = gap_map.get(subst_root).get(i1).get(i_inter).keySet().iterator();
				...
				if (gap_map.get(subst_root).get(i1).get(i_inter).get(j_inter).get(j2) == null)
				    continue;
				...
				double hyp = 1e300;
				if (gap_map.get(cur_node).get(i_inter).get(i2).get(j1).get(j_inter) != null)
				    hyp = gap_map.get(cur_node).get(i_inter).get(i2).get(j1).get(j_inter)[1]
					+ gap_map.get(subst_root).get(i1).get(i_inter).get(j_inter).get(j2)[0]
					+ 1.0; //this is the tree weight
	...
	//each map is indexed as [start of span1-1][end of span1][start of span2-1][end of span2][adjunction performed]
	...
	while (its.hasNext()) {
		...
		for (TagNode cur_node : tree_nodes) {
		    ...
		    closed_map.put(cur_node,new HashMap<Integer,Map<Integer,double[]> >());
		}
	    }
	    else {
		...
		for (TagNode cur_node : tree_nodes) {
			...
			gap_map.put(cur_node,new HashMap<Integer,Map<Integer, Map<Integer, Map<Integer, double[] > > > >());
	...
	while (its.hasNext()) {
		...
		if (cur_node.getType() == TagNode.LEX) {
			...
			for (int i=0; i <= nTokens; i++) {
			    ...
			    closed_map.get(cur_node).put(i, new HashMap<Integer,double[]>());
			    closed_map.get(cur_node).get(i).put(i,new double[2]);
			...
			for (int i=0; i < nTokens; i++) {
				...
				closed_map.get(cur_node).put(i, new HashMap<Integer,double[]>());
				closed_map.get(cur_node).get(i).put(i+1,new double[2]);
				closed_map.get(cur_node).get(i).get(i+1)[0] = 0.0;
				closed_map.get(cur_node).get(i).get(i+1)[1] = 1e300;
			    }
			}
		    }
		}
			...
			gap_map.get(cur_node).put(i,new HashMap<Integer, Map<Integer, Map<Integer, double[] > > >());
			gap_map.get(cur_node).get(i).put(i,new HashMap<Integer, Map<Integer, double[] > >());
			    
			for (int j=i; j <= nTokens; j++) {
			    //gap_map.get(cur_node)[i][i][j][j][0] = 0.0;

			    gap_map.get(cur_node).get(i).get(i).put(j,new HashMap<Integer, double[] >());
			    gap_map.get(cur_node).get(i).get(i).get(j).put(j,new double[2]);
			    gap_map.get(cur_node).get(i).get(i).get(j).get(j)[0] = 0.0;
			    gap_map.get(cur_node).get(i).get(i).get(j).get(j)[1] = 1e300;
			}
		    }
		}
	    }
	...
	for (int span = 0; span <= nTokens; span++) {
		...
		for (int pass = 0; next_pass_needed; pass++) {
			...
			if (gap_map.get(cur_node) != null) {
			    ...
			    Map<Integer, Map<Integer,Map<Integer,Map<Integer,double[]> > > > cur_gap_map 
				= gap_map.get(cur_node);
				...
				for (int j_inner = i_inner; j_inner <= j_outer; j_inner++) {
					...
					if (cur_gap_map.get(i_outer) != null && cur_gap_map.get(i_outer).get(i_inner) != null
					    && cur_gap_map.get(i_outer).get(i_inner).get(j_inner) != null
					    && cur_gap_map.get(i_outer).get(i_inner).get(j_inner).get(j_outer) != null) 
					    initial_score = cur_gap_map.get(i_outer).get(i_inner).get(j_inner).get(j_outer)[1];
					...
					if (nChildren > 0) {
						...
						Map<Integer, Map<Integer,Map<Integer,Map<Integer,double[]> > > > child_gap_map 
						    = gap_map.get(child);
						
						assert(gap_map.get(child) != null);

						if (child_gap_map.get(i_outer) != null 
						    && child_gap_map.get(i_outer).get(i_inner) != null
						    && child_gap_map.get(i_outer).get(i_inner).get(j_inner) != null
						    && child_gap_map.get(i_outer).get(i_inner).get(j_inner).get(j_outer) != null) { 
						
						    double hyp = child_gap_map.get(i_outer).get(i_inner).get(j_inner).get(j_outer)[0];
						...
						Map<Integer, Map<Integer,Map<Integer,Map<Integer,double[]> > > > child1_gap_map 
						    = gap_map.get(child1);
						...
						if (child1_gap_map != null) {						  
						    ...
						    if (child1_gap_map.get(i_outer) != null
							&& child1_gap_map.get(i_outer).get(i_inner) != null
							&& child1_gap_map.get(i_outer).get(i_inner).get(j_inner) != null) {

							assert(gap_map.get(child2) == null);

							Map<Integer,Map<Integer,double[]> > child2_closed_map  = closed_map.get(child2);
							
							Iterator<Integer> it = child1_gap_map.get(i_outer).get(i_inner).get(j_inner).keySet().iterator();
							...
							while(it.hasNext()) {
							    ...
							    if (child2_closed_map.get(split_point) != null
								&& child2_closed_map.get(split_point).get(j_outer) != null) {
								
								// double hyp = gap_map.get(child1)[i_outer][i_inner][j_inner][split_point][0]
								// 	+ closed_map.get(child2)[split_point][j_outer][0];
								double hyp = child1_gap_map.get(i_outer).get(i_inner).get(j_inner).get(split_point)[0]
								    + child2_closed_map.get(split_point).get(j_outer)[0];
						    ...
						    Map<Integer,double[]> child1_closed_map_i_outer = closed_map.get(child1).get(i_outer);
							...
							Map<Integer, Map<Integer,Map<Integer,Map<Integer,double[]> > > > child2_gap_map 
							    = gap_map.get(child2);						
							
							Iterator<Integer> it = child1_closed_map_i_outer.keySet().iterator();
							...
							while(it.hasNext()) {
							    ...
							    if (child2_gap_map.get(split_point) != null
								&& child2_gap_map.get(split_point).get(i_inner) != null
								&& child2_gap_map.get(split_point).get(i_inner).get(j_inner) != null
								&& child2_gap_map.get(split_point).get(i_inner).get(j_inner).get(j_outer) != null) {
							
								// double hyp = closed_map.get(child1)[i_outer][split_point][0]
								// 	+ gap_map.get(child2)[split_point][i_inner][j_inner][j_outer][0];
								
								double hyp = child1_closed_map_i_outer.get(split_point)[0]
								    + child2_gap_map.get(split_point).get(i_inner).get(j_inner).get(j_outer)[0];
					...
					if (best_adjpos_score < initial_score) {
					    ...
					    if (cur_gap_map.get(i_outer).get(i_inner) == null) {
						cur_gap_map.get(i_outer).put(i_inner, 
									     new HashMap<Integer, Map<Integer, double[] > >());
					    }
					    if (cur_gap_map.get(i_outer).get(i_inner).get(j_inner) == null) {

						cur_gap_map.get(i_outer).get(i_inner).put(j_inner, 
											  new HashMap<Integer, double[] >());
					    }
					    if (cur_gap_map.get(i_outer).get(i_inner).get(j_inner).get(j_outer) == null) {

						cur_gap_map.get(i_outer).get(i_inner).get(j_inner).put(j_outer, new double[2]);
						cur_gap_map.get(i_outer).get(i_inner).get(j_inner).get(j_outer)[0] = 1e300;
					    }

					    cur_gap_map.get(i_outer).get(i_inner).get(j_inner).get(j_outer)[1] = best_adjpos_score;
					...
					if (cur_gap_map.get(i_outer) != null && cur_gap_map.get(i_outer).get(i_inner) != null
					    && cur_gap_map.get(i_outer).get(i_inner).get(j_inner) != null
					    && cur_gap_map.get(i_outer).get(i_inner).get(j_inner).get(j_outer) != null) 
					    initial_score = cur_gap_map.get(i_outer).get(i_inner).get(j_inner).get(j_outer)[0];
					...
					if (cur_node.getAdjStatus() != TagNode.MADJ) {
						...
						double hyp_na = gap_map.get(cur_node).get(i_outer).get(i_inner).get(j_inner).get(j_outer)[1];
					...
					//DEBUG
					// if (i_outer == 1 && j_outer == 4 && i_inner == 4 && j_inner == 4) {
					    
					//     System.err.println("$$$$$in node: " + cur_node);
					// }
					//END_DEBUG
					...
					if (nChildren >  0 && !cur_node.isNoadj()) {
						...
						if (true) {
						    ...
						    Map<Integer,Map<Integer,Map<Integer,double[]> > > subst_gap_map_i_outer 
							= gap_map.get(subst_root).get(i_outer);

						    if (subst_gap_map_i_outer != null) {

							Iterator<Integer> it_i_inter = subst_gap_map_i_outer.keySet().iterator();
						    
							while (it_i_inter.hasNext()) {
							    
							    int i_inter = it_i_inter.next().intValue();
							    if (i_inter > i_inner)
								break;

							    if (cur_gap_map.get(i_inter) == null
								|| cur_gap_map.get(i_inter).get(i_inner) == null
								|| cur_gap_map.get(i_inter).get(i_inner).get(j_inner) == null)
								continue;
							    
							    Iterator<Integer> it_j_inter = subst_gap_map_i_outer.get(i_inter).keySet().iterator();

							    while (it_j_inter.hasNext()) {
						    
								int j_inter = it_j_inter.next().intValue();
								if (j_inter < j_inner)
								    continue;
								if (j_inter > j_outer)
								    break;
								...
								if (subst_gap_map_i_outer.get(i_inter).get(j_inter).get(j_outer) != null
								    && cur_gap_map.get(i_inter).get(i_inner).get(j_inner).get(j_inter) != null) {
							    
								    double hyp = cur_gap_map.get(i_inter).get(i_inner).get(j_inner).get(j_inter)[1]
									+ subst_gap_map_i_outer.get(i_inter).get(j_inter).get(j_outer)[0]
									+ 1.0;
					...
					if (best_adjdone_score < initial_score) {
						...
						cur_gap_map.get(i_outer).get(i_inner).get(j_inner).put(j_outer, new double[2]);
						cur_gap_map.get(i_outer).get(i_inner).get(j_inner).get(j_outer)[1] = 1e300;
					    }

					    cur_gap_map.get(i_outer).get(i_inner).get(j_inner).get(j_outer)[0] = best_adjdone_score;
			    ...
			    Map<Integer,Map<Integer,double[]> > cur_closed_map = closed_map.get(cur_node);
				...
				if (cur_closed_map.get(i_outer) != null && cur_closed_map.get(i_outer).get(j_outer) != null)
				    initial_score = cur_closed_map.get(i_outer).get(j_outer)[1];
				...
				if (nChildren > 0) {
					...
					if (closed_map.get(child).get(i_outer) != null && 
					    closed_map.get(child).get(i_outer).get(j_outer) != null) {
					
					    double hyp = closed_map.get(child).get(i_outer).get(j_outer)[0];
					...
					Map<Integer,Map<Integer,double[]> > child1_closed_map = closed_map.get(child1);
					
					Map<Integer,double[]>  child1_submap = child1_closed_map.get(i_outer);
					...
					if (child1_submap != null) {
					    ...
					    Iterator<Integer> it = child1_submap.keySet().iterator(); 
						...
						if (child2_closed_map.get(split_point) != null
						    ...
						    double hyp = child1_submap.get(split_point)[0]
							+ child2_closed_map.get(split_point).get(j_outer)[0];
				...
				if (best_adjpos_score < initial_score) {
					...
					cur_closed_map.put(i_outer,new HashMap<Integer, double[]>() );
				    }
				    if (cur_closed_map.get(i_outer).get(j_outer) == null) {
					cur_closed_map.get(i_outer).put(j_outer,new double[2]);
					cur_closed_map.get(i_outer).get(j_outer)[0] = 1e300;
				    }

				    cur_closed_map.get(i_outer).get(j_outer)[1] = best_adjpos_score;
				...
				if (cur_closed_map.get(i_outer) != null && cur_closed_map.get(i_outer).get(j_outer) != null)
				    initial_score = cur_closed_map.get(i_outer).get(j_outer)[0];
				...
				//CROSS-CHECK: is this the correct check for mandatory adjunction?
				if (cur_node.getAdjStatus() != TagNode.MADJ
				    && cur_closed_map.get(i_outer) != null && cur_closed_map.get(i_outer).get(j_outer) != null) {
				    //adjunction is not obligatory
				    double hyp_na = closed_map.get(cur_node).get(i_outer).get(j_outer)[1];
				    if (hyp_na < best_adjdone_score) {
				...
				if (nChildren == 0) {
					...
					if (true) {
					    ...
					    Map<Integer,Map<Integer,double[]> > subst_closed_map = closed_map.get(subst_root);

					    if (subst_closed_map.get(i_outer) != null && 
						subst_closed_map.get(i_outer).get(j_outer)  != null) {

						//NOTE: every time we integrate a tree into another tree, we add the weight
						// of the tree. This way the system with the passes is less complicated.
						// In the end we have to add the weight of the initial tree
						double hyp = closed_map.get(subst_root).get(i_outer).get(j_outer)[0]
						    + 1.0; //this is the tree weight
				    ...
				    //NOTE: it would be sensible to have a map of the labels to the corresponding auxiliary trees
					...
					if (true) {
					    ...
					    Map<Integer,Map<Integer,Map<Integer,double[]> > > subst_gap_map 
						= gap_map.get(subst_root).get(i_outer);

					    if (subst_gap_map == null)
						continue;

					    Iterator<Integer> it_i_inner = subst_gap_map.keySet().iterator();
					    
					    while (it_i_inner.hasNext()) {

						int i_inner = it_i_inner.next().intValue();
						...
						if (cur_closed_map.get(i_inner) == null)
						    continue;

						Iterator<Integer> it_j_inner = subst_gap_map.get(i_inner).keySet().iterator();
						
						while (it_j_inner.hasNext()) {

						    int j_inner = it_j_inner.next().intValue();
						    
						    if (j_inner > j_outer)
							break;
						    
						    if (cur_closed_map.get(i_inner).get(j_inner) == null)
							continue;
						    ...
						    if (subst_gap_map.get(i_inner).get(j_inner).get(j_outer) != null) {
							
							double hyp = subst_gap_map.get(i_inner).get(j_inner).get(j_outer)[0]
							    + cur_closed_map.get(i_inner).get(j_inner)[1]
							    + 1.0;
				...
				if (best_adjdone_score < initial_score) {
				    ...
				    if (cur_closed_map.get(i_outer).get(j_outer) == null) {
					cur_closed_map.get(i_outer).put(j_outer,new double[2]);
					cur_closed_map.get(i_outer).get(j_outer)[1] = 1e300;
				    }

				    cur_closed_map.get(i_outer).get(j_outer)[0] = best_adjdone_score;
	...
	if (!closed_in_queue.get(node).get(i).get(j)[pos]) {
		...
		if (gap_in_queue.get(foot_node).get(i) == null ) {
		    gap_in_queue.get(foot_node).put(i, new HashMap<Integer, Map<Integer, Map<Integer, boolean[]> > >() );
		    gap_map.get(foot_node).put(i, new HashMap<Integer, Map<Integer, Map<Integer, double[]> > >() );
		}
		if (gap_in_queue.get(foot_node).get(i).get(i) == null ) {
		    gap_in_queue.get(foot_node).get(i).put(i, new HashMap<Integer, Map<Integer, boolean[]> >() );
		    gap_map.get(foot_node).get(i).put(i, new HashMap<Integer, Map<Integer, double[]> >() );
		}
		if (gap_in_queue.get(foot_node).get(i).get(i).get(j) == null ) {
		    gap_in_queue.get(foot_node).get(i).get(i).put(j, new HashMap<Integer, boolean[] >() );
		    gap_map.get(foot_node).get(i).get(i).put(j, new HashMap<Integer, double[] >() );
		}

		if (gap_in_queue.get(foot_node).get(i).get(i).get(j).get(j) == null ) {

		    gap_in_queue.get(foot_node).get(i).get(i).get(j).put(j, new boolean[2] );
		    gap_map.get(foot_node).get(i).get(i).get(j).put(j, new double[2] );

		    gap_map.get(foot_node).get(i).get(i).get(j).get(j)[0] = 0.0;
		    gap_map.get(foot_node).get(i).get(i).get(j).get(j)[1] = 1e300;
	...
	//System.err.println("+++++ evaluating for queue: span ["+i1 + "," + i2 + "][" + j1 + "," + j2 + "]");
	//System.err.println("+++ node " + node);
       

	if (gap_in_queue.get(node).get(i1) == null ) {
	    gap_in_queue.get(node).put(i1, new HashMap<Integer, Map<Integer, Map<Integer, boolean[]> > >() );
	}
	if (gap_in_queue.get(node).get(i1).get(i2) == null ) {
	    gap_in_queue.get(node).get(i1).put(i2, new HashMap<Integer, Map<Integer, boolean[]> >() );
	}
	if (gap_in_queue.get(node).get(i1).get(i2).get(j1) == null ) {
	    gap_in_queue.get(node).get(i1).get(i2).put(j1, new HashMap<Integer, boolean[]>() );
	}
	if (gap_in_queue.get(node).get(i1).get(i2).get(j1).get(j2) == null ) {
	    gap_in_queue.get(node).get(i1).get(i2).get(j1).put(j2, new boolean[2] );
	...
	if (!gap_in_queue.get(node).get(i1).get(i2).get(j1).get(j2)[pos]) {
		...
		if (gap_in_queue.get(foot_node).get(i1) == null ) {
		    gap_in_queue.get(foot_node).put(i1, new HashMap<Integer, Map<Integer, Map<Integer, boolean[]> > >() );
		    gap_map.get(foot_node).put(i1, new HashMap<Integer, Map<Integer, Map<Integer, double[]> > >() );
		}
		if (gap_in_queue.get(foot_node).get(i1).get(i1) == null ) {
		    gap_in_queue.get(foot_node).get(i1).put(i1, new HashMap<Integer, Map<Integer, boolean[]> >() );
		    gap_map.get(foot_node).get(i1).put(i1, new HashMap<Integer, Map<Integer, double[]> >() );
		}
		if (gap_in_queue.get(foot_node).get(i1).get(i1).get(j2) == null ) {
		    gap_in_queue.get(foot_node).get(i1).get(i1).put(j2, new HashMap<Integer, boolean[] >() );
		    gap_map.get(foot_node).get(i1).get(i1).put(j2, new HashMap<Integer, double[] >() );
		}

		if (gap_in_queue.get(foot_node).get(i1).get(i1).get(j2).get(j2) == null ) {

		    gap_in_queue.get(foot_node).get(i1).get(i1).get(j2).put(j2, new boolean[2] );
		    gap_map.get(foot_node).get(i1).get(i1).get(j2).put(j2, new double[2] );

		    gap_map.get(foot_node).get(i1).get(i1).get(j2).get(j2)[0] = 0.0;
		    gap_map.get(foot_node).get(i1).get(i1).get(j2).get(j2)[1] = 1e300;
	 ...
	 *** [Kallmeyer, Parsing beyond context free grammars, Springer (2010), pages 78-82].
	 *** At the same time, this is an agenda-driven routine where impossible items are never visited.
	 ***/
	...
	while (its.hasNext()) {
		...
		if (foot_nodes_ij.get(label) == null)
		    foot_nodes_ij.put(label, new Vector<Vector<List<TagNode> > >());
		if (subst_nodes_ij.get(label) == null)
		    subst_nodes_ij.put(label, new Vector<Vector<List<TagNode> > >());
		...
		for (TagNode cur_node : tree_nodes) {
		    ...
		    closed_map.put(cur_node,new HashMap<Integer,Map<Integer,double[]> >());
		    closed_in_queue.put(cur_node,new HashMap<Integer,Map<Integer,boolean[]> >());
		}
	    }
	    else {
		...
		for (TagNode cur_node : tree_nodes) {
			...
			gap_map.put(cur_node,new HashMap<Integer,Map<Integer, Map<Integer, Map<Integer, double[] > > > >());
			gap_in_queue.put(cur_node,new HashMap<Integer,Map<Integer, Map<Integer, Map<Integer, boolean[] > > > >());
	...
	for (int k=0; k <= nTokens; k++) {
		...
		subst_nodes_ij.get(label_it.next()).add(new Vector<List<TagNode> >() );

	    label_it = foot_nodes_ij.keySet().iterator();
	    while(label_it.hasNext())
		foot_nodes_ij.get(label_it.next()).add(new Vector<List<TagNode> >() );
		...
		if (k2 < k) {
		    ...
		    label_it = subst_nodes_ij.keySet().iterator();
		    while(label_it.hasNext())
			subst_nodes_ij.get(label_it.next()).get(k).add(new LinkedList<TagNode>() );

		    label_it = foot_nodes_ij.keySet().iterator();
		    while(label_it.hasNext())
			foot_nodes_ij.get(label_it.next()).get(k).add(new LinkedList<TagNode>() );
		}
	    }
	}
	...
	while (its.hasNext()) {
		...
		if (cur_node.getType() == TagNode.LEX) {
			...
			for (int i1= 0; i1 <= nTokens; i1++) 
				...
				foot_nodes_ij.get(cur_label).get(i1).get(i2).add(cur_node);
		    }
		    else {
		    ...
		    // foot predicts are no longer added from the start, they are now handled via the agenda
		    // for (int i=0; i <= nTokens; i++) {
			...
			for (int i1 = 0; i1 <= nTokens; i1++) 
			    for (int i2 = i1; i2 <= nTokens; i2++)
				subst_nodes_ij.get(cur_node.getCategory()).get(i1).get(i2).add(cur_node);
		    }
		    else {
	...
	long chartInitTime = System.nanoTime() - chartStartTime;
	System.err.println("Total time for init & chart initialization: " + (chartInitTime)/(Math.pow(10, 9))+" sec.");
	...
	while (active_list <= nTokens) {
	    ...
	    // if (cur_item.i2 != -1) {
	    // 	System.err.println("******* printing cur item: span ["+cur_item.i1 + "," + cur_item.i2 + "][" + cur_item.j1 + "," + cur_item.j2 + "]");
		...
		Map<Integer, Map< Integer, double[]> > cur_map = closed_map.get(cur_node);
		...
		if (pos == 1) {
			...
			//since i_outer and j_outer are really i and j, we don't have to check for existence here
			double prev_score = cur_map.get(i_outer).get(j_outer)[0];
			...
			for (TagTree subst_tag_tree : auxiliary_trees.get(category)) {
				...
				|| gap_map.get(subst_root).get(i_outer).get(i) == null
				|| gap_map.get(subst_root).get(i_outer).get(i).get(j) == null
				|| gap_map.get(subst_root).get(i_outer).get(i).get(j).get(j_outer) == null)
				continue;

			    double hyp_score = base_score + gap_map.get(subst_root).get(i_outer).get(i).get(j).get(j_outer)[0]
				+ 1.0; //this is the tree weight
			...
			if (siblings.size() == 1) {
			    ...
			    Map<Integer, Map< Integer, double[]> > par_map = closed_map.get(parent);


			    if (par_map.get(i) == null) {
				par_map.put(i, new HashMap< Integer, double[]>() );
			    }
				...
				if (closed_map.get(sibling) != null) {
				    ...
				    if (closed_map.get(sibling).get(j) != null) {

					Iterator<Integer> it = closed_map.get(sibling).get(j).keySet().iterator();
					...
					while (it.hasNext()) {
					    ...
					    double hyp = base_score + closed_map.get(sibling).get(j).get(j_outer)[0];
					    
					    if (par_map.get(i) == null)
						par_map.put(i, new HashMap<Integer, double[]>());
					    if (par_map.get(i).get(j_outer) == null) {
						par_map.get(i).put(j_outer, new double[2]);
						par_map.get(i).get(j_outer)[0] = 1e300;
						par_map.get(i).get(j_outer)[1] = 1e300;
					    }

					    if (hyp < par_map.get(i).get(j_outer)[1]) {
						
						par_map.get(i).get(j_outer)[1] = hyp;
				    ...
				    Map<Integer, Map<Integer, Map<Integer, Map< Integer, double[]> > > > par_map = gap_map.get(parent);

				    Iterator<Integer> it_i = gap_map.get(sibling).get(j).keySet().iterator();
					...
					Iterator<Integer> it_j = gap_map.get(sibling).get(j).get(i_inner).keySet().iterator();
					...
					while (it_j.hasNext()) {
					    ...
					    Iterator<Integer> it_j_outer = gap_map.get(sibling).get(j).get(i_inner).get(j_inner).keySet().iterator();

					    while (it_j_outer.hasNext()) {

						int j_outer = it_j_outer.next().intValue();

						if (j_outer > limit)
						    break;

						double hyp = base_score + gap_map.get(sibling).get(j).get(i_inner).get(j_inner).get(j_outer)[0];
						
						if (par_map.get(i) == null)
						    par_map.put(i, new HashMap<Integer, Map<Integer, Map< Integer, double[]> > >() );
						if (par_map.get(i).get(i_inner) == null)
						    par_map.get(i).put(i_inner, new HashMap<Integer, Map< Integer, double[]> >() );
						if (par_map.get(i).get(i_inner).get(j_inner) == null)
						    par_map.get(i).get(i_inner).put(j_inner, new HashMap<Integer, double[]>() );
						if (par_map.get(i).get(i_inner).get(j_inner).get(j_outer) == null) {
						    par_map.get(i).get(i_inner).get(j_inner).put(j_outer, new double[2]);
						    par_map.get(i).get(i_inner).get(j_inner).get(j_outer)[0] = 1e300;
						    par_map.get(i).get(i_inner).get(j_inner).get(j_outer)[1] = 1e300;
						}


						if (hyp < par_map.get(i).get(i_inner).get(j_inner).get(j_outer)[1]) {

						    par_map.get(i).get(i_inner).get(j_inner).get(j_outer)[1] = hyp;
				...
				if (closed_map.get(sibling) != null) {
				    ...
				    Iterator<Integer> it = closed_map.get(sibling).keySet().iterator();
					...
					if (closed_map.get(sibling).get(i_outer).get(i) == null)
					    continue;

					double hyp = base_score + closed_map.get(sibling).get(i_outer).get(i)[0];
					
					if (par_map.get(i_outer) == null)
					    par_map.put(i_outer, new HashMap<Integer, double[]>() );

					if (par_map.get(i_outer).get(j) == null) {
					    par_map.get(i_outer).put(j, new double[2]);
					    par_map.get(i_outer).get(j)[0] = 1e300;
					    par_map.get(i_outer).get(j)[1] = 1e300;
					}

					if (hyp < par_map.get(i_outer).get(j)[1]) {
					    
					    par_map.get(i_outer).get(j)[1] = hyp;
				    ...
				    Map<Integer, Map<Integer, Map<Integer, Map< Integer, double[]> > > > par_map = gap_map.get(parent);
				    Map<Integer, Map<Integer, Map<Integer, Map< Integer, double[]> > > > sib_map = gap_map.get(sibling);

				    int limit = Math.max(0,i-active_list);

				    Iterator<Integer> it_i_outer = sib_map.keySet().iterator();

				    while (it_i_outer.hasNext()) {

					int i_outer = it_i_outer.next().intValue();
					...
					Iterator<Integer> it_i = sib_map.get(i_outer).keySet().iterator();
					...
					while (it_i.hasNext()) {
					    ...
					    Iterator<Integer> it_j = sib_map.get(i_outer).get(i_inner).keySet().iterator();
						...
						if (sib_map.get(i_outer).get(i_inner).get(j_inner).get(i) == null)
						    continue;

						double hyp = base_score + sib_map.get(i_outer).get(i_inner).get(j_inner).get(i)[0];
						
						if (par_map.get(i_outer) == null)
						    par_map.put(i_outer, new HashMap<Integer, Map<Integer, Map< Integer, double[]> > >());
						if (par_map.get(i_outer).get(i_inner) == null)
						    par_map.get(i_outer).put(i_inner, new HashMap<Integer, Map< Integer, double[]> >());
						if (par_map.get(i_outer).get(i_inner).get(j_inner) == null)
						    par_map.get(i_outer).get(i_inner).put(j_inner, new HashMap< Integer, double[]>());
						if (par_map.get(i_outer).get(i_inner).get(j_inner).get(j) == null) {
						    par_map.get(i_outer).get(i_inner).get(j_inner).put(j, new double[2]);
						    par_map.get(i_outer).get(i_inner).get(j_inner).get(j)[0] = 1e300;
						    par_map.get(i_outer).get(i_inner).get(j_inner).get(j)[1] = 1e300;
						}

						if (hyp < par_map.get(i_outer).get(i_inner).get(j_inner).get(j)[1]) {

						    par_map.get(i_outer).get(i_inner).get(j_inner).get(j)[1] = hyp;
			...
			for (TagNode subst_node : subst_nodes_ij.get(label).get(i).get(j)) {
				...
				Map<Integer, Map< Integer, double[]> > subst_map = closed_map.get(subst_node);
				
				if (subst_map.get(i) == null) 
				    subst_map.put(i, new HashMap<Integer, double[]>() );
		...
		Map<Integer, Map< Integer, Map<Integer, Map<Integer, double[] > > > > cur_map = gap_map.get(cur_node);

		gap_in_queue.get(cur_node).get(i1).get(i2).get(j1).get(j2)[pos] = false;

		double base_score = cur_map.get(i1).get(i2).get(j1).get(j2)[pos];
		...
		if (pos == 1) {
			...
			if (base_score < cur_map.get(i1).get(i2).get(j1).get(j2)[0]) {

			    cur_map.get(i1).get(i2).get(j1).get(j2)[0] = base_score;
			...
			double prev_score = cur_map.get(i_outer).get(i2).get(j1).get(j_outer)[0];
			double best_score = prev_score;
			...
			for (TagTree subst_tag_tree : auxiliary_trees.get(category)) {
			    ...
			    Map<Integer, Map< Integer, Map<Integer, Map<Integer, double[] > > > > adj_map = gap_map.get(subst_root);

			    if (adj_map.get(i_outer) == null
				|| adj_map.get(i_outer).get(i1) == null
				|| adj_map.get(i_outer).get(i1).get(j2) == null
				|| adj_map.get(i_outer).get(i1).get(j2).get(j_outer) == null)
				continue;

			    //since i_outer and j_outer are really i1 and j2, we need not check for existence here

			    double hyp_score = base_score + adj_map.get(i_outer).get(i1).get(j2).get(j_outer)[0]
				+ 1.0; //this is the tree weight
			...
			if (best_score < prev_score) {
			    ...
			    cur_map.get(i_outer).get(i2).get(j1).get(j_outer)[0] = best_score;
			...
			if (siblings.size() == 1) {
				...
				par_map.put(i1, new HashMap< Integer, Map<Integer, Map<Integer, double[] > > >() );
			    if (par_map.get(i1).get(i2) == null)
				par_map.get(i1).put(i2, new HashMap< Integer, Map<Integer, double[] > >() );
			    if (par_map.get(i1).get(i2).get(j1) == null)
				par_map.get(i1).get(i2).put(j1, new HashMap< Integer, double[]>() );
			    if (par_map.get(i1).get(i2).get(j1).get(j2) == null) {
				par_map.get(i1).get(i2).get(j1).put(j2, new double[2] );
				par_map.get(i1).get(i2).get(j1).get(j2)[0] = 1e300;
				par_map.get(i1).get(i2).get(j1).get(j2)[1] = 1e300;
			    }

			    if (base_score < par_map.get(i1).get(i2).get(j1).get(j2)[1] ) {

				par_map.get(i1).get(i2).get(j1).get(j2)[1] = base_score;
				...
				if (closed_map.get(sibling).get(j2) != null) {
				    ...
				    Iterator<Integer> it = closed_map.get(sibling).get(j2).keySet().iterator();
					...
					double hyp = base_score + closed_map.get(sibling).get(j2).get(j_outer)[0];
					...
					if (par_map.get(i1).get(i2).get(j1) == null)
					    par_map.get(i1).get(i2).put(j1, new HashMap< Integer, double[]>() );
					if (par_map.get(i1).get(i2).get(j1).get(j_outer) == null) {
					    par_map.get(i1).get(i2).get(j1).put(j_outer, new double[2] );
					    par_map.get(i1).get(i2).get(j1).get(j_outer)[0] = 1e300;
					    par_map.get(i1).get(i2).get(j1).get(j_outer)[1] = 1e300;
					}
	
					if (hyp < par_map.get(i1).get(i2).get(j1).get(j_outer)[1]) {
					    
					    par_map.get(i1).get(i2).get(j1).get(j_outer)[1] = hyp;
				...
				while (it.hasNext()) {
				    ...
				    if (closed_map.get(sibling).get(i_outer).get(i1) == null)
					continue;

				    double hyp = base_score + closed_map.get(sibling).get(i_outer).get(i1)[0];

				    if (par_map.get(i_outer) == null)
					par_map.put(i_outer, new HashMap< Integer, Map<Integer, Map<Integer, double[] > > >() );
				    if (par_map.get(i_outer).get(i2) == null)
					par_map.get(i_outer).put(i2, new HashMap< Integer, Map<Integer, double[] > >() );
				    if (par_map.get(i_outer).get(i2).get(j1) == null)
					par_map.get(i_outer).get(i2).put(j1, new HashMap< Integer, double[]>() );
				    if (par_map.get(i_outer).get(i2).get(j1).get(j2) == null) {
					par_map.get(i_outer).get(i2).get(j1).put(j2, new double[2] );
					par_map.get(i_outer).get(i2).get(j1).get(j2)[0] = 1e300;
					par_map.get(i_outer).get(i2).get(j1).get(j2)[1] = 1e300;
				    }
				    
				    if (hyp < par_map.get(i_outer).get(i2).get(j1).get(j2)[1]) {

					par_map.get(i_outer).get(i2).get(j1).get(j2)[1] = hyp;
			...
			for (TagNode adj_node : adjunction_nodes.get(cur_node.getCategory())) {

			    //System.err.println("*************** checking adjunction of node " + cur_node + " to node " + adj_node );
			    //System.err.println("spans: ["+i1+","+i2+"]["+j1+","+j2+"]");
				...
				if (closed_map.get(adj_node) != null) {
					...
					&& closed_map.get(adj_node).get(i2).get(j1) != null) {

					double hyp = base_score + closed_map.get(adj_node).get(i2).get(j1)[1]
					    + 1.0; //tree weight

					if (closed_map.get(adj_node).get(i1) == null)
					    closed_map.get(adj_node).put(i1, new HashMap<Integer, double[]>() );
					if (closed_map.get(adj_node).get(i1).get(j2) == null) {
					    closed_map.get(adj_node).get(i1).put(j2, new double[2] );
					    closed_map.get(adj_node).get(i1).get(j2)[0] = 1e300;
					    closed_map.get(adj_node).get(i1).get(j2)[1] = 1e300;
					}
				    
					if (hyp < closed_map.get(adj_node).get(i1).get(j2)[0]) {
					
					    closed_map.get(adj_node).get(i1).get(j2)[0] = hyp;
				    ...
				    Map<Integer, Map< Integer, Map<Integer, Map<Integer, double[] > > > > adj_map = gap_map.get(adj_node);
				    
				    if (adj_map.get(i2) == null)
					continue;

				    Iterator<Integer> it_i = adj_map.get(i2).keySet().iterator();
					...
					Iterator<Integer> it_j = adj_map.get(i2).get(i_inner).keySet().iterator();
					...
					while (it_j.hasNext()) {
					    ...
					    if (adj_map.get(i2).get(i_inner).get(j_inner).get(j1) == null)
						continue;

					    double hyp = base_score + adj_map.get(i2).get(i_inner).get(j_inner).get(j1)[1]
						+ 1.0; //tree weight
					    
					    if (adj_map.get(i1) == null)
						adj_map.put(i1, new HashMap< Integer, Map<Integer, Map<Integer, double[] > > >() );
					    if (adj_map.get(i1).get(i_inner) == null)
						adj_map.get(i1).put(i_inner, new HashMap< Integer, Map<Integer, double[] > >() );
					    if (adj_map.get(i1).get(i_inner).get(j_inner) == null)				    
						adj_map.get(i1).get(i_inner).put(j_inner, new HashMap<Integer, double[] >() );
					    if (adj_map.get(i1).get(i_inner).get(j_inner).get(j2) == null) {				    
						adj_map.get(i1).get(i_inner).get(j_inner).put(j2, new double[2] );
						adj_map.get(i1).get(i_inner).get(j_inner).get(j2)[0] = 1e300;
						adj_map.get(i1).get(i_inner).get(j_inner).get(j2)[1] = 1e300;
					    }
					    

					    if (hyp < adj_map.get(i1).get(i_inner).get(j_inner).get(j2)[0]) {

						adj_map.get(i1).get(i_inner).get(j_inner).get(j2)[0] = hyp;
	...
	// 		Iterator<Integer> it2 = closed_map.get(node).get(i).keySet().iterator();
			
	// 		while (it2.hasNext()) {

	// 		    int j = it2.next().intValue();

	// 		    System.err.println("["+i+","+j+"]: " + closed_map.get(node).get(i).get(j)[0] + "," 
	...
	// 		Iterator<Integer> it_i2 = gap_map.get(node).get(i1).keySet().iterator();
			
	// 		while (it_i2.hasNext()) {

	// 		    int i2 = it_i2.next().intValue();

	// 		    Iterator<Integer> it_j1 = gap_map.get(node).get(i1).get(i2).keySet().iterator();

	// 		    while (it_j1.hasNext()) {

	// 			int j1 = it_j1.next().intValue();

	// 			Iterator<Integer> it_j2 = gap_map.get(node).get(i1).get(i2).get(j1).keySet().iterator();
	...
	for (TagTree tag_tree : initial_trees.get(axiom)) {
		...
		if (closed_map.get(tree_root).get(0) != null
		    && closed_map.get(tree_root).get(0).get(nTokens) != null)
